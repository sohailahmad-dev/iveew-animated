import React, { useEffect, useState } from 'react';
import Btn from '../btn/Btn'
import './Testimonial.css';
import { Grid } from '@mui/material';
import ArrowBackIosIcon from '@mui/icons-material/ArrowBackIos';
import ArrowForwardIosIcon from '@mui/icons-material/ArrowForwardIos';
import TestimonialCard from './TestimonialCard';

export default function Testimonial({ slides }) {

    let [currentSlide, setCurrentSlide] = useState(1);
    const totalSlides = slides?.length;

    const changeSlide = () => {
        (currentSlide === totalSlides) ? setCurrentSlide(1) : setCurrentSlide(currentSlide + 1);
    }

    useEffect(() => {
        let interval = setInterval(changeSlide, 5000);
        return () => clearInterval(interval);
    })

    const handlePreviousSlide = () => {
        if (currentSlide === 1) {
            setCurrentSlide(totalSlides)
        } else {
            setCurrentSlide(currentSlide - 1)
        }
    }

    const handleNextSlide = () => {
        if (currentSlide === totalSlides) {
            setCurrentSlide(1)
        } else {
            setCurrentSlide(currentSlide + 1)
        }
    }


    return (
        <>
            <div className="testimonial-desktop" style={{ margin: '20px 100px' }}>
                <Grid container spacing={5}>
                    {slides[currentSlide - 1] && <Grid item xs={12} sm={4}>
                        <TestimonialCard content={slides[currentSlide - 1]} />
                    </Grid>}
                    {slides[currentSlide] && <Grid item xs={12} sm={4}>
                        <TestimonialCard content={slides[currentSlide]} />
                    </Grid>}
                    {slides[currentSlide + 1] && <Grid item xs={12} sm={4}>
                        <TestimonialCard content={slides[currentSlide + 1]} />
                    </Grid>}

                </Grid>
            </div>
            <div className="testimonial-mobile">
                <Grid container spacing={5}>
                    {slides[currentSlide - 1] && <Grid item xs={12} sm={4}>
                        <TestimonialCard content={slides[currentSlide - 1]} />
                    </Grid>}
                </Grid>
            </div>
            <div className="testimonial-navigation">
                <div onClick={handlePreviousSlide}><ArrowBackIosIcon /> </div>
                <div />
                <div />
                <div />
                <div />
                <div onClick={handleNextSlide}> <ArrowForwardIosIcon /> </div>
            </div>
            <Btn label='Realize seu orcamento' style={{ width: '341px', maxWidth: '100%', margin: '0px auto' }} />
        </>
    )
}
